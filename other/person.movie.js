    ...
    const promise = (async () => {
        try {
            ...
            if (params.filter?.trim()) {
                ...
                logger.info(`关键词/正则过滤完成，剩余: ${works.length}/${beforeFilterCount}`);
            }

            // 第7步：格式化输出
            const finalList = formatOutput(works);
            logger.info(`格式化完成，最终返回数量: ${finalList.length}`);
