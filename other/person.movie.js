...
WidgetMetadata = {
  ...
  description: "获取 TMDB 个人相关作品数据（只返回已上映作品）",
  author: "Evan (Optimized by ChatGPT)",
...
async function getAllWorks(params) {
  ...
  list = filterReleased(list);  // ✅ 过滤未播出
...
async function getOtherWorks(params) {
  var p = params || {};
  var credits = await fetchCredits(p.personId, p.language);
  var list = credits.crew.filter(i => {
    var job = i.job && i.job.toLowerCase();
